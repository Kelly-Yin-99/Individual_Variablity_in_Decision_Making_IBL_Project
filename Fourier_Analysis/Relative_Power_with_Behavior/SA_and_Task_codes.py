...
for region, region_data in all_results.items():
    for session in region_data:
        ...
        for period in ['task', 'SP']:
            ...
            if not spec:
                continue
            freqs = sorted([float(f.replace("Hz", "")) for f in spec if 2 <= float(f.replace("Hz", "")) <= 49])
            ...
            _, smoothed_log_power = smooth_sharp_jumps(freqs, log_power, delta=0.25)
...
pdf_path = "/Users/naokihiratani/Documents/Relative_Power_Task_SA_1-49hz_4-12.pdf"
...
pdf_path_subject = "/Users/naokihiratani/Documents/relative_power_subjectAvg_2-49hz_4-12.pdf"
